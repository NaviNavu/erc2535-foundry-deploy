...
abstract contract FacetHelper {
    ...
    function _getSelectorsFromArtifact(string memory _facetName) internal returns(bytes4[] memory selectors) {
        ...
        cmd[0] = "script/GenerateSelectorsByArtifact.sh";
        cmd[1] = _facetName;
    ...
    function _getFacetSelectorsFromLoupe(
        ...
        _facetFunctionSelectors = IDiamondLoupe(diamondAddress).facetFunctionSelectors(facetAddress);
    }
    ...
    function _getFacetBySelectorFromLoupe(
        ...
        _facetAddress = IDiamondLoupe(diamondAddress).facetAddress(functionSelector);
    }
}